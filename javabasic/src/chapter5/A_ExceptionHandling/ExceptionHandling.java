...
// 시스템 에러 : 개발자가 코드상으로 조취를 취할 수 없는 문제 (전원 부족, 메모리 부족 etc)
// 컴파일 에러 : 컴파일이 불가능한 상태 (오타, 참조 타입 불일치 etc)
// 런타임 에러 : 프로그램이 실행 도중 발생하는 문제 (빌드 실패, 포트 중복, 데이터베이스 연결실패)
//  로직 에러 : 프로그램의 아웃풋이 예상과 다르게 나오는 문제 
// 예외 : 정상적인 입력에서는 정상처리가 되지만 비정상적인 입력에서는 처리가 되지 않는 문제(null 값 참조, 로직 실행 중 데이터 벱이스 연결 실패)


public class ExceptionHandling {
	// throws 예외 클래스 : 해당 예외를 호출부로 책임을 전가
	// throws 가 선언부에 추가된 메서드를 호출하는 곳에서
	// 예외 처리에 대한 강제성이 부여됨
...
//		System.out.println(numbers[3]);
		...
		// 사용 방법
		// try{ 예외가 발생할 수 있는 문장}
		// catch (예외타입 예외 참조변수){예외가 발생했을 떄 처리할 문장}
